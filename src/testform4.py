...
def import_form3(file_path):
    df = pd.read_excel(file_path, sheetname="Tabelle1", skiprows=2, parse_cols=None, dtypes=str)
    df["testid"] = df["Verbindung"] + "_" + df["Bezeichnung"] + "_"  + df["Pruefart"] + "_"  + df["Fuegeverfahren"]
...
def import_ks2_series(file_path, testf4):
...
if __name__ == '__main__':
    ...
    # tp = sdata.testprogram.TestProgram.from_folder(exportpath)
    # print("tp", tp.group)
    # ts = tp.get_data_by_name("15_Scherzug_FLS_HC340LA_1,5_HC340LA_1,5_qs")
    # print("ts", ts, ts.group)

    df_fls = import_form3(file_path="/method/fosta/P1032-Einseitig_Zugänglich/1_Ergebnisse/Form3_FLS_02_05.xls")

    df_bs = import_form3(file_path="/method/fosta/P1032-Einseitig_Zugänglich/1_Ergebnisse/Form3_Bolzensetzen_09_05.xls")
    df = pd.concat([df_fls, df_bs], ignore_index=True)
    # print(df.describe())

    # df_ = df[(df["Verbindung"]=="V_1032_11") & (df["Bezeichnung"]=="LWF-KS-2-0Grad") & (df["Fuegeverfahren"]=="FLS")]
    # print(df_)
    #
    # df_ = df[(df["Verbindung"]=="V_1094_37") & (df["Bezeichnung"]=="LWF-KS-2-0Grad") & (df["Fuegeverfahren"]=="Bolzensetzen")]
    # print(df_)
    #
    # df_ = df[(df["Verbindung"]=="V_1094_37") & (df["Bezeichnung"]=="LWF-KS-2-0Grad") & (df["Fuegeverfahren"]=="FLS")]
    # print(df_)
    # row = df_.iloc[0]
    #print(df.ix[:, 30])
    df_ = df_fls.iloc[:, 103].dropna()
    # print (df_)

    failure_modes = {}

    for linenr, row in df_fls.iterrows():
        # print(row["Verbindung"], row[103])
        for i in range(5):
            # print(row["testid"], type(row["testid"]))
            if isinstance(row["testid"], float):
                continue
            testid = row["testid"] + ("_%02d"%(i+1))
